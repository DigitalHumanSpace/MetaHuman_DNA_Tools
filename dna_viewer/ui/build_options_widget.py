...
class BuildOptionsWidget(QWidget):
    ...
    def on_select_all_meshes_changed(self, state: int) -> None:
        """
        Method that gets called when the select all meshes checkbox is changed

        @type state: int
        @param state: The changed state of the checkbox
        """
        print(f"select all meshes changed to {state}")
        if state == Checkbox.checked.value:
            self.elements.mesh_tree_list.select_all_meshes()
        else:
            self.elements.mesh_tree_list.deselect_all_meshes()